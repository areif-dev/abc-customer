...
impl AbcCustomer {
    ...
    pub fn from_db_export(
        ...
        for (sku, base_product) in base_products {
            ...
            products.insert(sku, AbcCustomer::try_from((&base_product, posted_product))?);
        }
        Ok(products)
    }
}

impl TryFrom<(&IntermediateBaseProduct, &IntermediatePostedProduct)> for AbcCustomer {
...
impl AbcProductBuilder {
    ...
    pub fn with_upcs(self, upcs: Vec<Gtin>) -> Self {
        AbcProductBuilder { upcs, ..self }
    }

    /// Add a UPC to the list of UPCs for this product
    pub fn add_upc(self, upc: Gtin) -> Self {
    ...
    pub fn build(self) -> Result<AbcCustomer, AbcParseError> {
        Ok(AbcCustomer {
            sku: self
                .sku
                .clone()
...
impl From<AbcCustomer> for AbcProductBuilder {
    fn from(value: AbcCustomer) -> Self {
        AbcProductBuilder {
            sku: Some(value.sku()),
    ...
    fn parse_item_data(
        ...
        while let Some(row) = item_data.records().next() {
            ...
            let upcs: Vec<Gtin> = upc_str
                ...
                .filter_map(|s| {
                    if s.len() == 11 {
                        // Some ABC UPCs leave out the check digit, so make one up and let [`Gtin::from_str_nonstrict`] fix it
...
mod tests {
    ...
    fn test_parser() {
        ...
        let products = AbcCustomer::from_db_export(item_path, item_posted_path).unwrap();
        assert_eq!(
            products,
